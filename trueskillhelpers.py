...
def calc_dynamic_draw_probability(rating_group1, rating_group2, env=None):
    ...
    return env.draw_probability(trueskill.Rating(team_perf_vars[0]), trueskill.Rating(team_perf_vars[1]), env)
...
def factor_graph_logging(color=False):
    ...
    def factor_init(self, *args, **kwargs):
        ...
        assert layer_builder_name.startswith("build_") and layer_builder_name.endswith("_layer")
    ...
    def variable_set(self, val):
        ...
        old_messages = {fac: Gaussian(pi=msg.pi, tau=msg.tau) for fac, msg in self.messages.items()}
